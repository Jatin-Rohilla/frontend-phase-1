...
// Check if the api header is defined and true
if (pm.request.headers.has('api') && pm.request.headers.get('api') === 'true') {
    // Encrypt the request body using the encryptRequestFields function
    const originalBody = pm.request.body.raw;
    if (originalBody) {
        try {
            const encryptedBody = encryptRequestFields(JSON.parse(originalBody));
            pm.request.body.raw = JSON.stringify(encryptedBody);
        } catch (error) {
